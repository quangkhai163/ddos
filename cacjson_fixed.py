import json
def reply_json(update, data):
    update.message.reply_text(json.dumps(data, ensure_ascii=False))
...
def turn_on(update: Update, context: CallbackContext):
    ...
    if update.effective_user.id != ADMIN_MAIN_ID:
        reply_json(update, {"status": "info", "message": "❌ Chi admin chinh moi bat đuoc bot."})
        return
    bot_status = True
    save_json_file(BOT_STATUS_FILE, {"status": bot_status})
    reply_json(update, {"status": "info", "message": "✅ Bot đa bat."})

def turn_off(update: Update, context: CallbackContext):
    global bot_status
    if update.effective_user.id != ADMIN_MAIN_ID:
        reply_json(update, {"status": "info", "message": "❌ Chi admin chinh moi tat đuoc bot."})
        return
    bot_status = False
    save_json_file(BOT_STATUS_FILE, {"status": bot_status})
    reply_json(update, {"status": "info", "message": "⛔ Bot đa tat."})
...
def attack(update: Update, context: CallbackContext):
    ...
    if chat_id not in ALLOWED_GROUP_IDS:
        reply_json(update, {"status": "info", "message": "❌ Nhom chua đuoc duyet."})
        return
    if not bot_status:
        reply_json(update, {"status": "info", "message": "⛔ Bot đang tat."})
        return
    ...
    try:
        ...
        if is_blacklisted(url):
            reply_json(update, {"status": "info", "message": "🚫 URL nam trong blacklist."})
            return
        ...
        threading.Timer(duration, lambda: context.bot.send_message(chat_id=chat_id, text=json.dumps({"status": "info", "message": "✅ Đa hoan tat attack"}, ensure_ascii=False))).start()
    except Exception:
        reply_json(update, {"status": "info", "message": "❗ Sai cu phap. Vi du: /attack <url> <time>"})
...
def attackvip(update: Update, context: CallbackContext):
    ...
    if user_id not in ADMIN_IDS:
        reply_json(update, {"status": "info", "message": "🚫 Chi admin moi đuoc dung lenh nay."})
        return
    ...
    try:
        ...
        if method not in ["flood", "bypass"]:
            reply_json(update, {"status": "info", "message": "❌ Phuong thuc chi co flood hoac bypass."})
            return
        ...
        context.bot.send_message(chat_id=chat_id, text=msg, reply_markup=reply_markup)
        threading.Timer(duration, lambda: context.bot.send_message(chat_id=chat_id, text=json.dumps({"status": "info", "message": "✅ VIP attack hoan tat"}, ensure_ascii=False))).start()
    except Exception:
        reply_json(update, {"status": "info", "message": "❗ Sai cu phap. Vi du: /attackvip <url> <time> <flood|bypass>"})
...
def proxy(update: Update, context: CallbackContext):
    try:
        ...
        reply_json(update, {"status": "info", "message": "❌ Khong tim thay file proxy."})
...
def addadmin(update: Update, context: CallbackContext):
    if update.effective_user.id != ADMIN_MAIN_ID:
        reply_json(update, {"status": "info", "message": "❌ Chi admin chinh moi đuoc them admin."})
        return
    try:
        new_id = int(context.args[0])
        if new_id in ADMIN_IDS:
            reply_json(update, {"status": "info", "message": "ℹ️ ID nay đa la admin."})
            return
        ...
        reply_json(update, {"status": "info", "message": "❗ Sai cu phap. Vi du: /addadmin <id>"})

def deladmin(update: Update, context: CallbackContext):
    if update.effective_user.id != ADMIN_MAIN_ID:
        reply_json(update, {"status": "info", "message": "❌ Chi admin chinh moi đuoc xoa admin."})
        return
    try:
        rem_id = int(context.args[0])
        if rem_id not in admin_data["sub_admins"]:
            reply_json(update, {"status": "info", "message": "ℹ️ ID khong phai admin phu."})
            return
        ...
        reply_json(update, {"status": "info", "message": "❗ Sai cu phap. Vi du: /deladmin <id>"})
...
def addgr(update: Update, context: CallbackContext):
    if update.effective_user.id != ADMIN_MAIN_ID:
        reply_json(update, {"status": "info", "message": "❌ Chỉ admin chính mới thêm nhóm."})
        return
    try:
        new_gid = int(context.args[0])
        if new_gid in ALLOWED_GROUP_IDS:
            reply_json(update, {"status": "info", "message": "ℹ️ Nhóm đã được duyệt trước đó."})
            return
        ...
        name = chat.title or "Không rõ"
        username = f"@{chat.username}" if chat.username else "(không có)"
        desc = chat.description or "(không có mô tả)"
        invite = getattr(chat, "invite_link", "(chưa tạo link)")
        count = context.bot.get_chat_members_count(new_gid)
        slow_mode = getattr(chat, "slow_mode_delay", 0)
        protected = "✅ Có" if getattr(chat, "has_protected_content", False) else "❌ Không"
        sticker_set = getattr(chat, "sticker_set_name", "(không có)")
        ...
        msg = (
            ...
            f"🐢 Slow mode: {slow_mode} giây\n"
            f"🛡 Bảo vệ nội dung: {protected}\n"
            f"🎨 Bộ sticker: {sticker_set}"
        )

        update.message.reply_text(msg)

    except Exception as e:
        update.message.reply_text(f"❌ Lỗi hoặc không tìm thấy nhóm: {e}")

def delgr(update: Update, context: CallbackContext):
    if update.effective_user.id != ADMIN_MAIN_ID:
        reply_json(update, {"status": "info", "message": "❌ Chi admin chinh moi xoa nhom."})
        return
    try:
        gid = int(context.args[0])
        if gid not in ALLOWED_GROUP_IDS:
            reply_json(update, {"status": "info", "message": "ℹ️ Nhom khong ton tai trong danh sach."})
            return
        ...
        reply_json(update, {"status": "info", "message": "❗ Sai cu phap. Vi du: /delgr <group_id>"})

def listgr(update: Update, context: CallbackContext):
    if update.effective_user.id != ADMIN_MAIN_ID:
        reply_json(update, {"status": "info", "message": "❌ Chi admin chinh moi xem danh sach nhom."})
        return
    if not ALLOWED_GROUP_IDS:
        reply_json(update, {"status": "info", "message": "ℹ️ Chua co nhom nao đuoc duyet."})
        return
...
# --- CHAY BOT ---
def main():
    updater = Updater(BOT_TOKEN, update_queue=None, use_context=True)
